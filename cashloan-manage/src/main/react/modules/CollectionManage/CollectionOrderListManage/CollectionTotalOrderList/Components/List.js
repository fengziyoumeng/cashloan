  ...
  render() {
    ...
    var columns = [{
      ...
      render(text, record) {
        ...
        //如果还没催收且已经还款则由管理员分配成员
        if(record.state == "40"&&(record.userName==null||record.userName=="")){
              return <div ><a href="#" onClick={me.showModalAc.bind(me,"分配催收员",record)}>分配催收员</a></div>
          }else{
          return "-"
        }
    ...
    return (
      <div className="block-panel">
           ...
           <Table columns={columns} rowKey={this.rowKey}  
             ...
             record={state.selectedrecord} dataRecord={state.dataRecord}  canEdit={state.canEdit} /> 
         </div>
    );
  }
})