...
public class JipushService {
    ...
    public static final Logger LOGGER = LoggerFactory.getLogger(JipushService.class);

    private final static String appKey = Global.getValue("appKey");

    private final static String masterSecret = Global.getValue("masterSecret");
    ...
    public int sendToAllAndroid( String notification_title, String msg_title, String msg_content, String extrasparam) {
        ...
        try {
            ...
            LOGGER.info("发送给所有安卓用户，发送前信息："+pushPayload);
            PushResult pushResult=jPushClient.sendPush(pushPayload);
            LOGGER.info("发送给所有安卓用户，发送完返回结果："+pushResult);
            if(pushResult.getResponseCode()==200){
                result=1;
            }
        } catch (Exception e) {
    ...
    public int sendToAll( String notification_title, String msg_title, String msg_content, String extrasparam) {
        ...
        try {
            ...
            LOGGER.info("发送给所有平台用户，发送前信息"+pushPayload);
            PushResult pushResult=jPushClient.sendPush(pushPayload);
            LOGGER.info("发送给所有平台用户，发送完返回结果"+pushResult);
            if(pushResult.getResponseCode()==200){
                result=1;
            }
        } catch (Exception e) {
